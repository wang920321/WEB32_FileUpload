...
public class FileuploadServlet2 extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean multipartContent = upload.isMultipartContent(request);//判断表单是否是文件上传表单
		if(multipartContent){
			//是文件上传表单
			//***解析request获得文件项集合
			try {
				...
				if(parseRequest!=null){
					for (FileItem fileItem : parseRequest) {
						...
						if(formField){
							...
							//当表单是  enctype="multipart/form-data" 时request.getParameter相关的方法都失效
			...
			//不是文件上传表单
			//使用原始的表单数据获取方式request.getParameter();
		}
	}
